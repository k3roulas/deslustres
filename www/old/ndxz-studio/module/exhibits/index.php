...
class Exhibits extends Router
{
	...
	function page_settings()
	{
		...
		if ($default['reporting'] == TRUE)
		{
  		...
  		// PLN
		$body .= ips($this->lang->word('Nb Line Title'), 'input', 'obj_nblinetitle', $rs['obj_nblinetitle'],
				"maxlength='10'", 'text','');
		$body .= ips($this->lang->word('List Expo Pre Title'), 'input', 'obj_listexpopretitle', $rs['obj_listexpopretitle'],
				"maxlength='128'", 'text','');
		$body .= ips($this->lang->word('List Expo Post Title'), 'input', 'obj_listexpoposttitle', $rs['obj_listexpoposttitle'],
				"maxlength='128'", 'text','');
		$body .= ips($this->lang->word('Height thumb'), 'input', 'obj_th2height', $rs['obj_th2height'],
				"maxlength='128'", 'text','');
	...
	function page_edit()
	{
		...
		$body .= "<label>Lien : </label>";
		$body .= "" . BASEURL . "/index.php?$rs[url]" . br();		
		$body .= "<label>Interne : </label>";
		$body .= "#" . str_replace ( "/", "-", $rs[url] )."" . br();
	...
	function page_section()
	{
		...
		$body .= "<h2>" . BASEURL . "/index.php?$rs[sec_path]</h2>" . br();
		...
		$body .= ips("Decoupage en Annees", 'getGeneric', 'sec_year', $rs['sec_year']);
	...
	function page_view()
	{
		...
		$body .= ips($this->lang->word('alt'), 'input', 'media_alt',
			$rs['media_alt'], "id='media_alt' maxlength='255'", 'text');			

		$body .= ips($this->lang->word('image hide'), 'checkbox', 'media_hide', $rs['media_hide'], " id='media_hide' ");			
		$body .= ips($this->lang->word('image rollback'), 'checkbox', 'media_rollback', $rs['media_rollback'], " id='media_rollback' ");			
		$body .= "<br>";
	...
	function sbmt_edit_sec()
	{
		...
		$clean['sec_year'] = $processor->process('sec_year',array('boolean'));
	...
	function sbmt_upd_view()
	{
		...
		$clean['media_alt'] = $processor->process('media_alt', array('notags'));
		$clean['media_hide'] = $processor->process('media_hide');
		$clean['media_rollback'] = $processor->process('media_rollback');
	...
	function sbmt_upd_settings()
	{
		...
		$clean['obj_nblinetitle'] = $processor->process('obj_nblinetitle', array('digit'));
		$clean['obj_listexpopretitle'] = $processor->process('obj_listexpopretitle', array('notags'));
		$clean['obj_listexpoposttitle'] = $processor->process('obj_listexpoposttitle', array('notags'));
		$clean['obj_th2height'] = $processor->process('obj_th2height', array('notags'));
...
<a href=\'<%baseurl%><plug:ndxz_rewriter url=\'/about-this-site/\' />\'>" . $this->lang->word('about this site') . "</a></p>";
	...
	function sbmt_img_upload()
	{
		...
		$prefs = $this->db->fetchRecord("SELECT * FROM ".PX."objects_prefs");
		...
		$IMG->th2height = (! empty($prefs['obj_th2height'])) ? $prefs['obj_th2height'] : 100;
	...
	function sbmt_upd_jximg()
	{
		...
		$clean['media_alt'] = ($_POST['u'] == '') ? '' : utf8Urldecode($_POST['u']);
		$clean['media_hide'] = ($_POST['y'] == 'true') ? 1 : 0;
		$clean['media_rollback'] = ($_POST['z'] == 'true') ? 1 : 0;
	...
	function sbmt_upd_jxdelimg()
	{
		...
		deleteImage($clean['media_id'], 'th2'); // thumbnail

...
function updateImage(ida)
{
	...
	var hide = (document.getElementById('media_hide').checked == true);
	var rollback = (document.getElementById('media_rollback').checked == true);
	var alt = encodeURIComponent( $('input#media_alt').val() );
	
	$.post('?a='+action, { upd_jximg : 'true', u : alt, v : title, x : caption, y : hide, z : rollback, id : ida }, 
		function(html) {
			//updating(html);
			getExhibit();
	});

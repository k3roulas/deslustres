...
class Media
{
	...
	*/
	function upload_image($maxwidth, $thumb=FALSE)
	{
		if (($maxwidth != 9999) || ($thumb == TRUE))
		{
			...
			// ajout pour acd (dans le cas ou on uploader une image 
			// de hauteur 550, mais dont la largeur est differente de 746
			// on copie l'image dans un rectangle de 746x550
			if (($this->size[1] == 550 ) && ($this->size[0] < 746)) {
				
				// generer une image en agrandissant le canvas à 746
				$output_image = imagecreatetruecolor(746, 550);				

				$white = imagecolorallocate($output_image, 255, 255, 255);				
				imagecolortransparent($output_image, $white);		
				
				imagefilledrectangle($output_image, 0, 0, 746, 550, $white);
				...
				{
					// voir be.php.net/manual/en/function.imagesavealpha.php
				...
				@imagecopyresampled($output_image,  $this->input_image, 0, 0, 0, 0,
					$this->size[0], $this->size[1], $this->size[0], $this->size[1]);
			...
			// PLN j'ajoute le rezising d'une image
			// get the new sizes
			$this->resizing_maxheight($this->th2height);
			...
			if ($thumb == TRUE) 
			{
				$this->image =  $this->path . 'th2-' . $this->filename;
			}
	...
	* @return array
	*/
	function resizing_maxheight($maxheight)
	{
		$height_percentage = $maxheight / $this->size[1];

		if ($this->size[1] > $maxheight)
		{
			$this->new_size['w'] = round($height_percentage * $this->size[0]);
			$this->new_size['h'] = round($height_percentage * $this->size[1]);			
		}

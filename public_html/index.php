...
if ( (!$rs) && ( "$uri" != "/" ) ) {
	$rs = $OBJ->db->fetchRecord("SELECT *
		FROM ".PX."sections, ".PX."objects_prefs
		WHERE sec_path = '$uri'");
}


$rs = $OBJ->db->fetchRecord("SELECT * FROM ".PX."objects_prefs");
$rs['uri'] = $uri;


// recherche si on veux afficher une section
//if ( "$uri" != "/" ) {
//	$rs_pln = $OBJ->db->fetchRecord("SELECT *
//		FROM ".PX."sections
//		WHERE sec_path = '$uri'");
 ////
//	//if ($rs_pln)
